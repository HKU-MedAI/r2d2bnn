...
class BPNEvaluator(Evaluator):
    ...
    def infer_embs(self, data, tp="normal"):
        ...
        with torch.no_grad():
            if tp == "normal":
                embs = [self.model(data) for _ in range(self.n_normal_samples)]
                embs = torch.cat(embs, dim=0)
                self.n_1 = embs.shape[0]
                self.p = embs.shape[1]
            elif tp == "testing":
                embs = [self.model(data) for _ in range(self.n_test_samples)]
    ...
    def plot_tpr_fpr(self):
        ...
        for i, cl in enumerate(classes):
            ...
            with open(results_path + "summary.txt", "a") as f:
                f.write(f"{cl}; {auc(FPRs, TPRs)}; {np.nanmax(list(pixel_auc.values()))}; {np.nanmax(list(pro_all.values()))}\n")
