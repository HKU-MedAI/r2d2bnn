...
class BPNMdisEvaluator(Evaluator):
    def __init__(self, config):
        ...
        self.models = [
            parse_bayesian_model(self.config_train).to(self.device)
    ...
    def compute_normal_posterior(self, model, normal_dataloader):
        ...
        for data_n, l_n in normal_dataloader:
            ...
            with torch.no_grad():
                maps_list = [model.inference(data_n) for _ in range(self.n_normal_samples)]

            embeddings = [self.infer_embs(maps_list, i, j)
                          for i in range(self.n_rows)
                          for j in range(self.n_cols)
                          ]
            # with Pool(1) as p:
            #     posteriors = p.map(self.compute_mean_cov, embeddings)
            posteriors = [self.compute_mean_cov(emb) for emb in embeddings]
    ...
    def infer_embs(self, maps_list, i, j, tp="normal"):
        ...
        for maps in maps_list:
            ...
            # feat = F.tanh(torch.from_numpy(feat)).cpu().numpy()
    ...
    def compute_pvalue(self, idx, args):
        ...
        mdis = tester.rht(lamb, mu, test_mu, n_1, n_2).item()
        return mdis
        ...
        for cl_idx, cl in enumerate(self.classes):
            ...
            for def_type in os.listdir(def_path):
                ...
                for idx, (ts, ts_gt, gt_label) in enumerate(test_dataloader):
                    ...
                    with torch.no_grad():
                        d = ts.to(self.device)
                        maps_list = [self.models[cl_idx].inference(d) for _ in range(self.n_test_samples)]

                    embeddings = [self.infer_embs(maps_list, i, j, tp="testing")
                                  for i in range(self.n_rows)
                                  for j in range(self.n_cols)
                                  ]
