...
class FrequentistEvaluator(Evaluator):
    def __init__(self, config):
        ...
        low = 5
        lambdas = [10 ** i for i in range(low, low + 8)] + \
                  [10 ** i / 2 for i in range(low, low + 8)] + \
                  [10 ** i / 4 for i in range(low, low + 8)]
        self.all_lambdas = np.sort(lambdas)
        self.lambdas_list = [
            self.all_lambdas[i:i + 6] for i in range(len(self.all_lambdas) - 5)
        ]
    ...
    def compute_normal_posterior(self, normal_dataloader):
        # Compute posterior from normal images
        for data_n, l_n in normal_dataloader:
            data_list = [
                data_n[:, :, i * self.window_size:(i + 1) * self.window_size,
                j * self.window_size:(j + 1) * self.window_size]
                for i in range(self.n_rows)
                for j in range(self.n_cols)
            ]

            embeddings = [self.infer_embs(data) for data in data_list]
            with Pool(4) as p:
                posteriors = p.map(self.compute_mean_cov, embeddings)

            return posteriors

    def infer_embs(self, data, tp="normal"):
        ...
        with torch.no_grad():
            if tp == "normal":
                ...
                self.n_1 = embs.shape[0]
                self.p = embs.shape[1]
            elif tp == "testing":
                embs = [self.model(data)]
                embs = torch.stack(embs, dim=1)  # Cat along the batch dimension
            else:
                raise ValueError("Type not supported")

        return embs.cpu().numpy()

    def compute_cov(self, embs, mean):
        return [
            np.matmul(
                (embs[i] - mean).reshape([-1, 1]),
                (embs[i] - mean).reshape([1, -1])
            )
    ...
    def compute_mean_cov(self, embs):
        ...
        cov = self.compute_cov(embs, mean)
        cov = np.stack(cov, axis=0).sum(0)
    ...
    def visualize_result(self, input_img, r_set,
                         score_map, pvalues, l2_distance, var_test_embs, cl, def_type,
        ...
        ax_img[1, 1].title.set_text('Embedding Var')
    ...
    def plot_tpr_fpr(self):
        ...
        summary_file = open(results_path + "summary.txt", "a")
        summary_file.write("Class; Image AUC; Pixel AUC; PROAUC")
        ...
        for i, cl in enumerate(classes):
            ...
            posteriors = self.compute_normal_posterior(normal_dataloader)
            ...
            for def_type in os.listdir(def_path):
                ...
                for idx, (ts, ts_gt, gt_label) in enumerate(test_dataloader):
                    ...
                    d = torch.stack([
                        ts[b, :, i * self.window_size:(i + 1) * self.window_size,
                        j * self.window_size:(j + 1) * self.window_size]
                        for b in range(batch_size)
                        for i in range(self.n_rows)
                        for j in range(self.n_cols)
                    ], dim=0)
                    em = self.infer_embs(d, tp="testing")
                    ...
                    for b in range(batch_size):
                        for i in range(self.n_rows):
                            for j in range(self.n_cols):
                                embs[b,  i * self.n_cols + j, :, :] = em[
                                    b * self.n_cols * self.n_rows + i * self.n_cols + j, :, :]
                    ...
                    for lambda_idx in range(len(self.lambdas_list)):
                        for b in range(ts.shape[0]):
                            ...
                            if "good" not in def_type:
                                ...
                                pro = compute_pro(def_type, ts_gt[b].type(torch.int).numpy(), np.expand_dims(score_map, 0))
            ...
            print(f"Image AUC:{auc(FPRs, TPRs)}; Pixel AUC: {np.nanmax(list(pixel_auc.values()))}; PRO: {np.nanmax(list(pro_all.values()))}")
            rf.write(f"Image AUC:{auc(FPRs, TPRs)}; Pixel AUC: {np.nanmax(list(pixel_auc.values()))}; PRO: {np.nanmax(list(pro_all.values()))}\n")
            summary_file.write(f"{cl}; {auc(FPRs, TPRs)}; {np.nanmax(list(pixel_auc.values()))}; {np.nanmax(list(pro_all.values()))}\n")
