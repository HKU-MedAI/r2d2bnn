...
class HorseshoeLinearLayer(nn.Module):
    """
    ...
    def __init__(self, in_features, out_features, priors):
        ...
        self.beta_rho = nn.Parameter(torch.ones([out_features, in_features]) * priors["beta_rho_scale"])
        ...
        self.log_tau_rho = nn.Parameter(torch.ones(in_features) * priors["log_tau_rho_scale"])
        ...
        self.bias_rho = nn.Parameter(torch.ones([1, out_features]) * priors["bias_rho_scale"])
        ...
        self.log_v_rho = nn.Parameter(torch.ones([1, 1]) * priors["log_v_rho_scale"])
